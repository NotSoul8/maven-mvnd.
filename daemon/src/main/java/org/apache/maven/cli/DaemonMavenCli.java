...
public class DaemonMavenCli
{
    ...
    void logging( CliRequest cliRequest )
    {
        ...
        {
            ch.qos.logback.classic.Logger stdout = ( ch.qos.logback.classic.Logger ) slf4jLoggerFactory
                    .getLogger( "stdout" );
            ch.qos.logback.classic.Logger stderr = ( ch.qos.logback.classic.Logger ) slf4jLoggerFactory
                    .getLogger( "stderr" );
     ...
     * @return               Value for -rf flag to resume build exactly from place where it failed ({@code :artifactId}
     *                       in
            ...
            throws Exception
    {
        ...
        if ( cliRequest.commandLine.hasOption( CLIManager.ALTERNATE_USER_TOOLCHAINS ) )
        {
            userToolchainsFile = new File(
                    cliRequest.commandLine.getOptionValue( CLIManager.ALTERNATE_USER_TOOLCHAINS ) );
